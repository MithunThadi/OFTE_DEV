...
public class VariablesSubstitution {
	...
	public String variableSubstitutor(Map<String, String> transferMetaDataMap,
		...
		for (int i = 0; i < pattern.length(); i++) {
			...
			if (pattern.charAt(i) == '#') {
				...
				if (pattern.contains("(")) {
					switch (pattern.substring(i + 2, pattern.indexOf("("))) {
						case "FileName" :
							pattern = replacer(pattern, FileName, i);
							System.out.println(pattern);
							break;
						case "FilePath" :
							pattern = replacer(pattern, FilePath, i);
							break;
						default :
							break;
					}
					...
					switch (pattern.substring(i + 2, pattern.indexOf("}"))) {
						case "FileName" :
							pattern = replace(pattern, FileName, i);
							System.out.println(pattern);
							break;
						case "FilePath" :
							pattern = replace(pattern, FilePath, i);
							break;
						default :
							break;
					}
	...
	public String replace(String sourceFilePattern, String fileNameORfilePath,
			int i) {
